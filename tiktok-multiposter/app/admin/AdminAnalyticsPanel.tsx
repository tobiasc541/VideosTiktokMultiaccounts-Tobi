...
export default function AdminAnalyticsPanel(props:Props){
  ...
  function exportImage(){const c=document.createElement("canvas");c.width=1080;c.height=1350;const x=c.getContext("2d");if(!x)return;const g=x.createLinearGradient(0,0,1080,1350);g.addColorStop(0,"#020607");g.addColorStop(.5,"#071416");g.addColorStop(1,"#020304");x.fillStyle=g;x.fillRect(0,0,1080,1350);const glow=x.createRadialGradient(820,230,20,820,230,520);glow.addColorStop(0,"rgba(96,255,246,.22)");glow.addColorStop(1,"rgba(0,0,0,0)");x.fillStyle=glow;x.fillRect(0,0,1080,760);x.strokeStyle="rgba(106,246,239,.35)";x.lineWidth=2;x.strokeRect(52,52,976,1246);x.fillStyle="#77f6f0";x.font="900 48px Arial";x.fillText("VYRAL",82,135);x.fillStyle="#697984";x.font="700 20px Arial";x.fillText(`${networkLabel.toUpperCase()} · ${periodLabel.toUpperCase()}`,82,185);x.fillStyle="#fff";x.font="900 76px Arial";x.fillText("IMPACTO",82,325);x.fillText("DEL PERÍODO",82,410);const cards=[["VISTAS",n(data.views)],["SEGUIDORES",`+${n(data.followers)}`],["ME GUSTA",n(data.likes)],["COMENTARIOS",n(data.comments)]];cards.forEach((a,i)=>{const col=i%2,row=Math.floor(i/2),cx=82+col*458,cy=520+row*190;x.fillStyle="rgba(255,255,255,.035)";x.fillRect(cx,cy,420,150);x.strokeStyle="rgba(119,246,240,.16)";x.strokeRect(cx,cy,420,150);x.fillStyle="#74838d";x.font="700 18px Arial";x.fillText(a[0],cx+26,cy+42);x.fillStyle="#fff";x.font="900 48px Arial";x.fillText(a[1],cx+26,cy+105);});x.fillStyle="#7bf7f1";x.font="800 22px Arial";x.fillText("VYRAL PERFORMANCE",82,1000);x.fillStyle="#fff";x.font="italic 36px Georgia";const words=phrase.split(" ");let line="",yy=1070;for(const w of words){const test=line+w+" ";if(x.measureText(test).width>880){x.fillText(line,82,yy);line=w+" ";yy+=48}else line=test}x.fillText(line,82,yy);x.fillStyle="#6f7d87";x.font="500 20px Arial";x.fillText("Reporte generado desde el panel VYRAL.",82,1235);const a=document.createElement("a");a.href=c.toDataURL("image/png");a.download=`vyral-impacto-${Date.now()}.png`;a.click();}

  return <section className="adminSection adminAnalytics" id="analytics">
