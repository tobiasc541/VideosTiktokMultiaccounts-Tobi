...
export default function CreatorShareCard({ code, name }: { code: string; name: string }) {
  function download() {
    ...
    drawVyralMark(ctx,92,92,76);
    ctx.fillStyle="#F4FFFF";ctx.font=`800 40px ${sans}`;ctx.fillText("VYRAL",188,145);
    ctx.fillStyle="#7b8994";ctx.font=`700 19px ${sans}`;ctx.fillText("CREATOR PROGRAM / ACCESO ESPECIAL",92,202);

    ctx.fillStyle="#ffffff";ctx.font='italic 76px Georgia, "Times New Roman", serif';ctx.fillText("Tu comunidad",92,382);
    ctx.font=`800 94px ${sans}`;ctx.fillText("50% OFF",92,480);
    ctx.fillStyle="#cfeeed";ctx.font=`600 31px ${sans}`;ctx.fillText("para empezar a multiplicar su alcance.",92,535);

    ctx.fillStyle="rgba(118,245,239,.07)";ctx.fillRect(92,620,896,220);ctx.strokeStyle="rgba(118,245,239,.30)";ctx.strokeRect(92,620,896,220);
    ctx.fillStyle="#6cebe5";ctx.font=`700 20px ${sans}`;ctx.fillText("CÓDIGO PROMOCIONAL",130,685);
    ctx.fillStyle="#ffffff";ctx.font=`800 82px ${sans}`;ctx.fillText(code,130,780);

    ctx.fillStyle="#788690";ctx.font=`600 23px ${sans}`;ctx.fillText(`Compartido por ${name || "Creator VYRAL"}`,92,930);
    ctx.fillStyle="#ffffff";ctx.font=`800 48px ${sans}`;ctx.fillText("Publicá una vez.",92,1045);
    ctx.font='italic 52px Georgia, "Times New Roman", serif';ctx.fillText("Multiplicá tu alcance.",92,1110);
    ctx.fillStyle="#70eee8";ctx.font=`700 22px ${sans}`;ctx.fillText("CREÁ · DISTRIBUÍ · CRECÉ",92,1200);
    ctx.fillStyle="#7f8c95";ctx.font=`500 19px ${sans}`;ctx.fillText("Beneficio sujeto a código vigente y condiciones del plan.",92,1248);
