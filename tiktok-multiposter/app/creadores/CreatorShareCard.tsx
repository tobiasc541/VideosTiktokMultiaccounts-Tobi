...
function drawVyralLogo(ctx: CanvasRenderingContext2D, x:number, y:number) {
  const g=ctx.createLinearGradient(x,y,x+64,y+76); g.addColorStop(0,"#AFFFFB"); g.addColorStop(.48,"#54F3EE"); g.addColorStop(1,"#22D5D0");
  ctx.save(); ctx.lineCap="square"; ctx.lineJoin="miter"; ctx.strokeStyle=g; ctx.lineWidth=17; ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x+32,y+66); ctx.lineTo(x+64,y); ctx.stroke(); ctx.restore();
  ctx.fillStyle="#F5F8F8";ctx.font='800 40px Inter, Arial, sans-serif';ctx.fillText("YRAL",x+91,y+48);
}
...
export default function CreatorShareCard({ code, name }: { code: string; name: string }) {
  function download() {
    ...
    drawVyralLogo(ctx,92,96);ctx.fillStyle="#7b8994";ctx.font=`700 19px ${sans}`;ctx.fillText("CREATOR PROGRAM / ACCESO ESPECIAL",92,202);
    ...
    ctx.fillStyle="#788690";ctx.font=`600 23px ${sans}`;ctx.fillText(`Compartido por ${name || "Creator VYRAL"}`,92,930);ctx.fillStyle="#ffffff";ctx.font=`800 48px ${sans}`;ctx.fillText("Publicá una vez.",92,1045);ctx.font='italic 52px Georgia, "Times New Roman", serif';ctx.fillText("Multiplicá tu alcance.",92,1110);ctx.fillStyle="#70eee8";ctx.font=`700 22px ${sans}`;ctx.fillText("CREÁ · DISTRIBUÍ · CRECÉ",92,1200);ctx.fillStyle="#7f8c95";ctx.font=`500 19px ${sans}`;ctx.fillText("Beneficio sujeto a código vigente y condiciones del plan.",92,1248);ctx.fillStyle="#596872";ctx.font=`500 17px ${sans}`;ctx.fillText("Powered by Tobias Carrizo",790,1248);
