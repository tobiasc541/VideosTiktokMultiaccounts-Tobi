...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!body.accountId || !body.caption || !body.privacyLevel || !body.videoSize) return NextResponse.json({ error: "Faltan datos de publicación." }, { status: 400 });
    await assertTikTokAccountOwner(session.userId, String(body.accountId));
    ...
    if (periodEndRaw && new Date(periodEndRaw).getTime() <= Date.now()) {
      const next = { ...meta } as Record<string, unknown>; delete next.plan; next.subscription_status = "expired";
    ...
    if (!sameBatch) {
      ...
      await client.auth.admin.updateUserById(session.userId, { user_metadata: { ...meta, video_usage_month: month, videos_used_month: used + 1, last_video_credit_fingerprint: fingerprint, last_video_credit_at: new Date().toISOString() } });
    }
    const result = await initVideoPost({ accountId: body.accountId, caption: String(body.caption), privacyLevel: String(body.privacyLevel), videoSize: Number(body.videoSize) });
  ...
  } catch (e: any) { return NextResponse.json({ error: e.message || "Error TikTok" }, { status: 500 }); }
}
