...
No inventes precio, métricas, testimonios ni características ausentes.\nINTEGRACIÓN DE CONVERSIÓN — REGLA DURA:\n- El destino final de las cuatro ideas es: "${conversionGoal}".\n- Tejé ese objetivo de forma orgánica como consecuencia natural de la narrativa.\n- PROHIBIDO mencionar, vender, ofrecer o insinuar el recurso/objetivo de conversión en el hook o en Slide 1.\n- Si slide_count >= 2, la primera aparición explícita del objetivo/recurso sólo puede ocurrir en el SLIDE FINAL / CTA.\n- Si slide_count == 1, mantené el hook principal libre de pitch y reservá la conversión para una línea de CTA claramente separada al cierre de la placa.\n- El campo cta debe ejecutar específicamente "${conversionGoal}". No fuerces comentarios por DM si ese no es el objetivo. Si el objetivo sí requiere entregar un recurso por DM, podés usar una palabra clave corta y natural.\n- El CTA nunca debe contradecir conversion_goal ni convertir una pieza de viralidad/seguidores en una venta artificial. Priorizá ideas vendibles pero genuinamente variadas.\nAUDIO DE APERTURA — OBLIGATORIO: para cada idea generá además el primer audio que el dueño deberá grabar para el agente de Instagram. Debe aparecer como audioOpening con exactamente estos campos: name, transcript, when, purpose, stage, allowed_origins. name debe comenzar SIEMPRE con "PRIMER AUDIO OBLIGATORIO — ". transcript debe ser un audio humano, breve y natural que reconozca el contexto del contenido y abra conversación con una sola pregunta; NO debe entregar todavía el recurso ni preguntar si quiere el link. when debe dejar explícito que se reproduce inmediatamente después de que una persona comenta la publicación. purpose debe indicar que abre la conversación antes del entregable. stage debe ser EXACTAMENTE "opening". allowed_origins debe ser EXACTAMENTE ["post_comment"]. Este audioOpening es el primer audio obligatorio del agente, no una sugerencia opcional.\nDevolvé un objeto JSON con una única clave "ideas". "ideas" debe ser un array de exactamente 4 objetos con: id, name, hook, angle, humanStyle, business, offer, audience, goal, tone, cta, count, audioOpening. goal solo puede ser ventas, mensajes, seguidores, trafico o educar. count debe ser exactamente ${slideCount}. angle debe respetar el formato de ${slideCount===1?'una sola imagen fija sin etiquetas Slide':'exactamente '+slideCount+' slides'}.`;
