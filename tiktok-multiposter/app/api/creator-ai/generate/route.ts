...
export async function POST(req:Request){
 ...
 try{
  ...
  const body=await req.json();const logo=typeof body.logo==="string"&&body.logo.startsWith("data:image/")?body.logo:"";const humanMode=body.humanMode===true;const personRefs=Array.isArray(body.personRefs)?body.personRefs.filter((x:any)=>typeof x==="string"&&x.startsWith("data:image/")).slice(0,5):[];const count=Math.min(7,Math.max(1,Number(body.count)||6));const single=!!body.slide;
  ...
  const generated=await Promise.all(slides.map(async(s:any,i:number)=>{
   ...
   if(editRefs.length){
    ...
    editRefs.forEach((dataUrl:string,refIndex:number)=>{
     const comma=dataUrl.indexOf(",");const header=comma>=0?dataUrl.slice(0,comma):"";const encoded=comma>=0?dataUrl.slice(comma+1):"";const mime=header.slice(5).split(";")[0];
     if(!encoded||!["image/png","image/jpeg","image/webp"].includes(mime))throw new Error("Una imagen de referencia no es válida.");
     const ext=mime==="image/jpeg"?"jpg":mime.split("/")[1];const bytes=Uint8Array.from(atob(encoded),ch=>ch.charCodeAt(0));
     form.append("image",new Blob([bytes],{type:mime}),refIndex===0&&logo?"brand-logo."+ext:"person-reference-"+refIndex+"."+ext);
    });
    imageBody=form;
   }else{
