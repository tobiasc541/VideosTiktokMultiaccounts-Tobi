...
export async function GET(){const gate=await allowed();if(gate.error)return gate.error;const apiKey=key();if(!apiKey)return NextResponse.json({error:"Falta HEYGEN_API_KEY."},{status:503});try{const r=await fetch(BASE+"/v2/avatars",{headers:{"x-api-key":apiKey,"accept":"application/json"},cache:"no-store"});const j=await r.json().catch(()=>({}));if(!r.ok)return NextResponse.json({error:j?.message||j?.error||"No se pudieron cargar los avatares."},{status:502});const avatars=(j?.data?.avatars||[]).filter((a:any)=>a?.avatar_id&&a?.preview_image_url).map((a:any)=>({id:a.avatar_id,name:a.avatar_name||"Avatar",gender:a.gender||"",image:a.preview_image_url,video:a.preview_video_url||null}));return NextResponse.json({avatars});}catch(e:any){return NextResponse.json({error:e?.message||"No se pudieron cargar los avatares."},{status:500})}}
