...
export async function POST(req:Request){
 ...
 try{const b=await req.json();const duration=Math.min(30,Math.max(10,Number(b.duration)||30));const prompt=`Actuás como director creativo de anuncios UGC para VYRAL. Generá 5 conceptos distintos para un Reel vertical de máximo ${duration} segundos. Producto/brief: ${String(b.product||"")}. Datos extraídos de la tienda: ${JSON.stringify(b.productInfo||{})}. URL: ${String(b.sourceUrl||"")}. Diseñá el guion pensando en escenas UGC donde el producto se vea, se sostenga, se use o aparezca en primeros planos; no plantees solamente una persona hablando a cámara. Público: ${String(b.audience||"")}. Idioma: ${String(b.language||"Español (Argentina)")}. Estilo: ${String(b.style||"UGC")}. Cada guion debe poder decirse naturalmente dentro del tiempo. No inventes precios, testimonios, métricas ni características no provistas. Respondé SOLO JSON array de 5 objetos {"id":1,"hook":"hook breve","angle":"nombre del enfoque","script":"guion hablado completo","cta":"CTA breve"}.`;
