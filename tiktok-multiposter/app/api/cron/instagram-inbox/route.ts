      ...
      try{
        ...
        try{
          ...
          }
          {
            ...
            if(asksResource&&resourcePool.length){const t=normVoiceText(userInput),tokens=t.split(" ").filter((x:string)=>x.length>3);let chosen:any=null,best=-1;for(const r of resourcePool){const hay=normVoiceText(`${r.name||""} ${r.purpose||""} ${r.send_when||""}`);let s=tokens.filter((w:string)=>hay.includes(w)).length;if(/prueba|resultado|backtest|win rate|winrate|estrategia solida/.test(t)&&/prueba|resultado|backtest|win rate|winrate|estrategia solida/.test(hay))s+=20;if(s>best){best=s;chosen=r}}if(chosen&&best>0){const resourceUrl=chosen.kind==="url"?String(chosen.external_url||""):await signedMedia(db,String(chosen.storage_path||""),604800);if(resourceUrl){if(chosen.kind==="url")await send(account,person,`${chosen.name||"Te dejo el link"}: ${resourceUrl}`);else await sendAttachment(account,person,resourceType(String(chosen.name||chosen.storage_path)),resourceUrl)}}}
