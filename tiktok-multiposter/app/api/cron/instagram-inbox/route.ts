...
async function aiReply(a:any,text:string){
  const key=process.env.OPENAI_API_KEY;if(!key)return String(a.dmMessage||"Gracias por escribir. ¿En qué te puedo ayudar?");
  const prompt=`Sos el agente de Instagram de este negocio. Continuá la conversación por DM de forma breve, natural y útil. Mensaje del usuario: ${text}. Contexto: ${String(a.contentLabel||"")}. Objetivo: ${String(a.conversationGoal||"lead")}. Tono: ${String(a.aiTone||"natural")}. País: ${String(a.aiCountry||"")}. Reglas: ${String(a.aiInstructions||"")}. No inventes precios, condiciones ni datos. Hacé como máximo una pregunta. Respondé SOLO el texto a enviar.`;
...
export async function GET(req:Request){
  ...
  for(const account of accounts.data||[]){
    ...
    const user=await db.auth.admin.getUserById(account.user_id),rules=(user.data?.user?.user_metadata?.vyral_automations||[]).filter((a:any)=>a.enabled&&a.platforms?.includes("instagram")&&a.continueConversation!==false&&a.dmEnabled!==false);
    for(const person of people){
      try{
        ...
        for(const m of messages){
          const mid=String(m.id||""),from=String(m.from?.id||""),body=String(m.message||"").trim();if(!mid||!body||from!==person)continue;
          const synthetic=`dm:${mid}`;const exists=await db.from("instagram_automation_runs").select("id").eq("comment_id",synthetic).maybeSingle();if(exists.data)continue;
          const prior=(recent.data||[]).find((x:any)=>String(x.commenter_id)===person),a=rules.find((x:any)=>x.id===prior?.automation_id)||rules[0];if(!a)continue;
          ...
          try{const reply=await aiReply(a,body);if(!reply)continue;const sent=await send(account,person,reply);await db.from("instagram_automation_runs").update({status:"sent",private_message_id:String(sent.message_id||"")||null,updated_at:new Date().toISOString(),detail:{source:"instagram_conversations_poll",continueConversation:true}}).eq("id",ins.data?.id);results.push({account:account.id,person,status:"sent"})}
