...
async function processMessage(db:any,account:any,m:any){
 ...
 try{
  ...
  const result=await processInstagramConversationEvent({account,automation:a,contactId:senderId,messageId:mid,text,origin:"direct_dm",contextPayload:{post_title:String(a.contentLabel||""),target_topic:String(a.contentLabel||""),media_id:String(prev?.media_id||"")},history,source:"webhook"});
...
async function processComment(db:any,account:any,value:any){const commentId=String(value?.id||""),text=String(value?.text||""),fromId=String(value?.from?.id||""),username=String(value?.from?.username||""),mediaId=String(value?.media?.id||"");if(!commentId||!text||fromId===String(account.instagram_user_id))return;
 ...
 let dm=String(decision.dm||a.dmMessage||"").trim();const firstVoice=chooseVoice(a,text,true),shortName=instagramFirstName(username);if(firstVoice?.url&&shortName&&a.personalizeName!==false)dm=shortName;else if(a.personalizeName!==false)dm=personalizeFirstDm(dm,username);
