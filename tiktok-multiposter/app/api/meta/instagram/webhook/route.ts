...
async function processMessage(db:any,account:any,m:any){
 ...
 const chosen=voiceResource||(decision.resourcePool||[]).find((r:any)=>String(r.id)===String(decision.resourceId))||((a.conversationMode==="instant"&&asksResource)?(decision.resourcePool||[])[0]:null),shouldResource=Boolean(chosen&&(voiceResource||decision.sendResource||(a.conversationMode==="instant"&&asksResource)));
 ...
 try{
  ...
  await db.from("instagram_automation_runs").update({status:"sent",private_message_id:String(last?.message_id||"")||null,updated_at:new Date().toISOString(),detail:{source:"instagram_dm",continueConversation:true,resourceSent:shouldResource,resourceTriggeredByVoice:Boolean(voiceResource),resourceId:chosen?.id||null,voiceSent:voice?.id||null,bubbles:messages.length}}).eq("id",ins.data?.id)
