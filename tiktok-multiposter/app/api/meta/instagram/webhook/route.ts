...
async function processMessage(db:any,account:any,m:any){
 ...
 if(voice){const audioUrl=await signedMedia(db,String(voice.url));if(audioUrl){try{await sendAttachment(account,{id:senderId},"audio",audioUrl)}catch{ /* keep the conversation alive if Instagram rejects this audio encoding */ }}}
