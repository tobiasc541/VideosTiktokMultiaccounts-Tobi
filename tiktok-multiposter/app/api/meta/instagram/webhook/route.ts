...
async function conversationReply(a:any,text:string,history:string="",firstName:string=""){
 ...
 const evidence=a.resourceUrl?{name:String(a.resourceName||"recurso"),description:String(a.resourcePurpose||""),when:String(a.resourceWhen||"")}:null;
...
Devolvé SOLO JSON válido: {"messages":["..."],"sendResource":false,"stage":"new|curious|qualified|hot|converted|support","reason":""}.`;
...
async function processMessage(db:any,account:any,m:any){
 ...
 const shouldResource=Boolean(a.resourceUrl&&(decision.sendResource||(a.conversationMode==="instant"&&asksResource)));
 ...
 try{
  ...
  if(shouldResource){const resourceUrl=await signedMedia(db,String(a.resourceUrl),604800);if(resourceUrl)await sendAttachment(account,{id:senderId},resourceType(String(a.resourceName||a.resourceUrl)),resourceUrl)}
