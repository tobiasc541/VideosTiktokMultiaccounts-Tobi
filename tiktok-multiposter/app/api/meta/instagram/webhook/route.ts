...
async function processMessage(db:any,account:any,m:any){
 ...
 const voice=chooseVoice(a,text,false);
 ...
 if(voice){const audioUrl=await signedMedia(db,String(voice.url));if(audioUrl)await sendAttachment(account,{id:senderId},"audio",audioUrl)}
...
async function processComment(db:any,account:any,value:any){const commentId=String(value?.id||""),text=String(value?.text||""),fromId=String(value?.from?.id||""),username=String(value?.from?.username||""),mediaId=String(value?.media?.id||"");if(!commentId||!text||fromId===String(account.instagram_user_id))return;
 ...
 if(a.dmEnabled&&(dm||firstVoice)){if(firstVoice){const audioUrl=await signedMedia(db,String(firstVoice.url));if(audioUrl){const j=await sendAttachment(account,{comment_id:commentId},"audio",audioUrl);messageId=String(j.message_id||"")}}else if(dm){const j=await ig(`${GRAPH}/${VER}/${encodeURIComponent(account.instagram_user_id)}/messages`,account.access_token,{recipient:{comment_id:commentId},message:{text:dm}});messageId=String(j.message_id||"")}}
