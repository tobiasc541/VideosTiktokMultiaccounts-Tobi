...
export async function POST(req:Request){
 ...
 try{
  ...
  const log=async(stage:string,detail:any={},containerId?:string,mediaId?:string)=>{await db.from("instagram_publish_events").insert({user_id:session.userId,account_id:accountId||null,container_id:containerId||null,media_id:mediaId||null,storage_path:storagePath||null,stage,detail});};
  await log("prepare_started",{fileName,mimeType,fileSize});
  ...
  await log("upload_authorized",{bucket:BUCKET,path:storagePath});return NextResponse.json({ok:true,action:"upload",bucket:BUCKET,path:storagePath,token:prep.data.token,signedUrl:prep.data.signedUrl});
...
export async function PUT(req:Request){
 ...
 try{
  ...
  const db=supabaseAdmin();
  const log=async(stage:string,detail:any={},containerId?:string,mediaId?:string)=>{await db.from("instagram_publish_events").insert({user_id:session.userId,account_id:accountId||null,container_id:containerId||null,media_id:mediaId||null,storage_path:path||null,stage,detail});};
  await log("finalize_started",{shareToFeed});
  ...
  const parent=path.split("/").slice(0,-1).join("/"),leaf=path.split("/").pop()||"";const exists=await db.storage.from(BUCKET).list(parent,{search:leaf,limit:10});if(exists.error)throw new Error(`No se pudo verificar el video: ${exists.error.message}`);if(!(exists.data||[]).some(x=>x.name===leaf)){await log("storage_missing",{parent,leaf});return NextResponse.json({error:"El video no llegó al almacenamiento de VYRAL. Volvé a intentar la subida."},{status:409});}await log("storage_confirmed",{parent,leaf});
