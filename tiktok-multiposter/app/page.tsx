...
export default async function Home() {
  ...
  if (periodEnd && new Date(String(periodEnd)).getTime() <= Date.now()) {
    const next = { ...meta } as Record<string, unknown>; delete next.plan; next.subscription_status = "expired"; next.account_notice = "Tu período terminó. Elegí un plan para volver a activar VYRAL.";
    await client.auth.admin.updateUserById(session.userId, { user_metadata: next }); redirect("/planes?expired=1");
  }
  ...
  return <div className={`dashboardPageWrap plan-${planId} ${starter ? "starterPlan" : "premiumPlan"}`}>
    ...
    <nav className="dashboardSideExtras" aria-label="Cuenta y soporte"><Link href="/mi-plan"><span>◇</span><b>Tu cuenta</b></Link><Link href="/creadores"><span>✦</span><b>Creator Viral</b></Link><Link href="/ayuda"><span>?</span><b>Preguntas frecuentes</b></Link><Link href="/soporte"><span>↗</span><b>Soporte 24/7</b></Link><CreditBadge /></nav>
    <section className="dashboardPostFlow"><div className={`dashboardTopContent ${starter ? "starterLockedFeature" : ""}`}><TopViralDemo />{starter && <div className="starterFeatureOverlay"><div className="starterGateIcon">✦</div><small>TOP CONTENT · PREMIUM</small><h2>Descubrí qué contenido está impulsando tu crecimiento.</h2><p>Los 5 videos más virales y sus métricas se desbloquean desde el plan Crecimiento.</p><Link href="/mi-plan#upgrade">Cambiar plan ↗</Link></div>}</div><div className="dashboardPostMetrics"><section className="vdCard"><div className="vdCardHead"><div><div className="vdLabel">RENDIMIENTO</div><h2>Últimos 30 días</h2></div></div><div className="vdMiniStats"><div><span>Vistas</span><b>284.750</b></div><div><span>Seguidores</span><b>+4.680</b></div><div><span>Compartidos</span><b>8.210</b></div></div></section><section className="vdCard"><div className="vdCardHead"><div><div className="vdLabel">IMPACTO VYRAL</div><h2>Distribución multicuentas</h2></div></div><div className="vdImpactCompare compact"><div><span>Mejor cuenta individual</span><b>118.400</b></div><div><span>Vistas adicionales</span><b>+166.350</b></div><div><span>Multiplicador</span><b>2,40×</b></div></div></section></div></section>
    <div className={starter ? "starterAnalyticsFilterLocked" : ""}><AnalyticsFilterPanel /></div><PremiumAnalyticsPanel planId={planId} />
  </div>;
}
