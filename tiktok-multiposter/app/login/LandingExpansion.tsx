...
const plans=[
 {name:"Inicio",price:"US$ 4,99",tag:"OPERACIÓN",items:["2 cuentas","50 videos / mes","Publicación multicuentas","Historial","Soporte 24/7"],off:["Analytics","VYRAL AI"]},
 {name:"Crecimiento",price:"US$ 9,99",tag:"CRECIMIENTO",items:["5 cuentas","200 videos / mes","Top 5 contenido","Analytics por red y período","Comparativas por cuenta","VYRAL AI","Soporte 24/7"],off:[]},
 {name:"Escala",price:"US$ 19,99",tag:"INTELLIGENCE",items:["30 cuentas","500 videos / mes","Todo Crecimiento","Viral Momentum Score","Content Intelligence","Recomendaciones accionables","Predicción de crecimiento","Patrones de captions y formatos"],off:[]}
];
...
export default function LandingExpansion(){
 return <div className="vyralLandingNext">
   ...
   <section className="nextSection nextCreator"><div className="nextCreatorGlow"/><div className="nextCreatorMesh"/><div><div className="nextIndex">08 / CREATOR PROGRAM</div><small>CRECÉ CON VYRAL</small><h2>Tu audiencia también<br/><em>puede convertirse en negocio.</em></h2><p>Solicitá tu código Creator, compartí un beneficio para tu comunidad y acumulá comisiones recurrentes mientras las suscripciones sigan activas.</p><div className="nextCreatorFlow"><span>VOS</span><i>→</i><span>TU CÓDIGO</span><i>→</i><span>COMUNIDAD</span><i>→</i><strong>INGRESO RECURRENTE</strong></div></div><div className="nextCreatorStats"><article className="benefit"><div className="nextBenefitSignal"><i/><span>BENEFICIO PARA TU COMUNIDAD</span><b>ACTIVO</b></div><div className="nextBenefitNumber"><strong>50</strong><div><b>%</b><span>OFF</span></div></div><small>para seguidores con código vigente</small><div className="nextBenefitBeam"/></article><article><span>COMISIÓN</span><strong>10%</strong><small>recurrente sobre suscripciones atribuidas</small></article><article><span>PANEL</span><strong>LIVE</strong><small>usuarios, facturación y comisión</small></article></div></section>

   <section className="nextSection nextPlans"><div className="nextPlansHead"><div><div className="nextIndex">09 / PLANES</div><small>EMPEZÁ SIMPLE. ESCALÁ CUANDO QUIERAS.</small><h2>Tres niveles.<br/><em>Una estrategia: crecer con control.</em></h2></div><p>Cada nivel suma profundidad sin complicar tu operación. Inicio distribuye. Crecimiento mide. Escala convierte datos en decisiones concretas.</p></div><div className="nextPlanGrid">{plans.map((p,i)=><article className={i===2?"hero":""} key={p.name}><span>{p.tag}</span><h3>{p.name}</h3><strong>{p.price}<small>/ mes</small></strong><div>{p.items.map(x=><p key={x}>✓ {x}</p>)}{p.off.map(x=><p className="off" key={x}>× {x}</p>)}</div><Link href="/planes">Empezar con {p.name} ↗</Link></article>)}</div></section>
   ...
   <footer className="nextFooter"><div className="nextFooterBrand"><img src="/vyral-logo.svg" alt="VYRAL"/><small>Powered by Tobias Carrizo</small></div><span>Distribución · Analytics · Intelligence</span><div><Link href="/terms">Términos</Link><Link href="/privacy">Privacidad</Link></div></footer>
 </div>
}
