...
type Pub={id:string;scheduled_at:string;timezone:string;caption:string;privacy_level?:string|null;platforms:string[];targets:Target[];status:string;last_error?:string|null;platform_results?:Record<string,unknown>;created_at:string;preview_url?:string|null;file_name?:string|null};
...
export default function PublicationHistory(){
 ...
 return <section className="vyralHistory">
  ...
  {loading?<div className="vhEmpty">Cargando historial…</div>:!shown.length?<div className="vhEmpty">Todavía no hay publicaciones en esta vista.</div>:<div className="vhGrid">{shown.map(x=><article className="vhCard" key={x.id}>
   <div className="vhPreview">{x.preview_url?<video src={x.preview_url} muted playsInline preload="metadata"/>:<div><span>▶</span><small>{x.file_name||"VIDEO VYRAL"}</small></div>}<b className={`vhStatus ${x.status}`}>{labels[x.status]||x.status}</b></div>
