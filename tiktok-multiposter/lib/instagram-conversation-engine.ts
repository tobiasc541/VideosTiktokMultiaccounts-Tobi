...
export async function processInstagramConversationEvent(event:InstagramConversationEvent){
 ...
 // STRICT DELIVERY DEDUPE:
 // resources_sent is authoritative for physical delivery within the activation.
 // A delivered resource can be sent again ONLY when semantic intent says the user reports it missing / asks for a resend.
 ...
 const actionResource=plannedResource&&(!alreadyDelivered||explicitRetry)&&(currentTurnRequiresDelivery||!alreadyDelivered)?plannedResource:null;
